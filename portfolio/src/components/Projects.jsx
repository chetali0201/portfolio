...
import fraudImg from '../assets/project/AI.png'
import cafeImg from '../assets/project/cafe.png'
import travelImg from '../assets/project/travel.png'
...
const FILTERS = ['All', 'AI / ML', 'Full Stack', 'Frontend']

/* PROJECT IMAGE COMPONENT */
function ProjectArt({ variant, image }) {
  const gradients = {
    fraud: 'from-rose-500/30 via-ink-800 to-signal-cyan/20',
    cafe: 'from-orange-400/30 via-ink-800 to-yellow-300/20',
    travel: 'from-sky-400/30 via-ink-800 to-cyan-300/20',
    portfolio: 'from-violet-500/30 via-ink-800 to-signal-cyan/20',
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-mono text-white/40 text-xs tracking-[0.3em]">
          PROJECT
        </span>
      </div>
    </div>
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-4 py-2 rounded-full transition ${
              filter === f ? 'bg-signal-cyan text-black' : 'glass'
            }`}
          >
            {f}
          </button>
        ...
        <AnimatePresence>
          {filtered.map((p) => (
            ...
            >
              <ProjectArt
                variant={p.image}
                image={PROJECT_IMAGES[p.image]}
              />

              <h3 className="mt-5 text-xl font-semibold">{p.title}</h3>

              <p className="mt-3 text-paper/60">{p.desc}</p>
